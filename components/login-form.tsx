...
function LoginFormContent({ className, ...props }: React.ComponentProps<"div">) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    console.log("Connexion soumise ! Email:", email, "Mot de passe:", password);
  };
...
function SignUpFormContent() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const username = (e.currentTarget.elements.namedItem('signup-username') as HTMLInputElement).value;
    ...
    const confirmPassword = (e.currentTarget.elements.namedItem('signup-confirm-password') as HTMLInputElement).value;

    if (password !== confirmPassword) {
      alert("Les mots de passe ne correspondent pas !");
      return;
    }
    console.log("Inscription soumise ! Nom:", username, "Email:", email, "Mot de passe:", password);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <Input id="signup-username" type="text" placeholder="Votre nom" required />
      </div>
      <div>
...
// --- Composant principal AuthCard ---
export default function AuthCard() {
  const [mode, setMode] = useState('login');

  return (
    <div className="items-center justify-center flex shadow-xl rounded-xl bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 w-[480px] h-[700px] rounded-lg">
    <Card className="w-[380px]">
      ...
      <div className="text-muted-foreground *:[a]:hover:text-primary text-center text-xs text-balance *:[a]:underline *:[a]:underline-offset-4 px-6 pb-6">
        <a href="https://www.tiktok.com/@mrdahel?lang=fr">Je suis un debutant en nextjs, abonnez vous pour me donner des feedbacks</a>{" "}
